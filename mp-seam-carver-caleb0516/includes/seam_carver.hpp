...
public:
  ...
  void Clear(int** energy_matrix) const;
  SeamCarver() = default;
  ...
  // int GetSquared(int before_square);
  //  returns the energy of the pixel at row col in image_
  int GetEnergy(int row, int col) const;
  int** GetEnergyMatrix() const;

  int** CalculatingVerticalSeamMatrix() const;
  int** CalculatingHorizontalSeamMatrix() const;
  // int** CalculatingHorizontalSeamMatrix()const;  // take the energy matrix
  // and create
  //  the addition matrix
  // tkae the seam from the addition matrix.
  // findinveritcal seam
