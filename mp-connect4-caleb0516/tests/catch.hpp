...
namespace Catch {
  namespace Benchmark {
    namespace Detail {
      ...
      /// measurement. Implementations of std::function with optimizations that
      /// aren't always applicable, like small buffer optimizations, are not
      /// uncommon. This is effectively an implementation of std::function
      /// without any such optimizations; it may be slow, but it is consistently
      /// slow.
...
namespace Catch {
  namespace Benchmark {
    namespace Detail {
      ...
      TimingOf<Clock, Fun, int> measure_one(Fun&& fun,
                                            int iters,
                                            std::false_type) {
    ...
    namespace Detail {
      ...
      bootstrap_analysis analyse_samples(double confidence_level,
        ...
        auto n = static_cast<int>(last -
                                  first);  // seriously, one can't use integral
                                           // types without hell in C++
...
namespace Catch {
  ...
  // MINSIGSTKSZ; Since glibc 2.33.9000 MINSIGSTKSZ is no more a constant.  So,
